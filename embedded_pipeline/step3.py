...
classifier_interpreter = tf.lite.Interpreter(model_path="LIS-Detection/final_project/lis_dnn_model_quant_int8.tflite")
...
while True:
    ...
    # === Converti keypoints e visualizza
    input_keypoints = []
    points = []
    ...
    for i in range(21):
        ...
        points.append((x, y))
        input_keypoints.append(x_norm * 256)
        input_keypoints.append(y_norm * 256)
    ...
    # === Classificazione LIS
    input_array = np.array([input_keypoints], dtype=np.float32)
    input_array = input_array - 128  # centra su 0
    input_array = np.clip(input_array, -128, 127).astype(np.int8)

    classifier_interpreter.set_tensor(classifier_input[0]['index'], input_array)
    classifier_interpreter.invoke()
    output = classifier_interpreter.get_tensor(classifier_output[0]['index'])[0]

    predicted_class = int(np.argmax(output))
    letter = chr(ord('A') + predicted_class)

    # === Mostra predizione
    cv2.putText(frame_copy, f"Pred: {letter}", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 255), 2)
    print(f"[LIS] Lettera predetta: {letter}")
